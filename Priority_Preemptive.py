from PyQt5.QtWidgets import QVBoxLayout, QWidget, QErrorMessage, QMessageBox, QFrame, QLabel, QLineEdit, QPushButton, \
...
class PriorityPreemptiveClass(QWidget):
    def __init__(self):
        ...
        self.tableWidget.setHorizontalHeaderItem(3, QTableWidgetItem("Priority"))

        self.main_label = QLabel("Priority (Preemptive)")
    ...
    def add_btn_click(self):
        if self.arrival_time_input.text().isdigit() and self.burst_time_input.text().isdigit() and self.priority_input.text().isdigit() and self.count < int(
                self.Process_number_input.text()):
            self.sorted_list.insert(self.Process_name_input.text(), int(self.arrival_time_input.text()),
            ...
            for row in range(self.count):
                ...
                self.tableWidget.setItem(row, 3, QTableWidgetItem(str(self.sorted_list.list[row].priority)))
    ...
    def plot_btn_click(self):
        ...
        if self.count == int(self.Process_number_input.text()):
            self.sorted_list.preemptivePriority()
            ...
            self.show_error("please enter remaining processes")

        self.plot_graph.reset()
